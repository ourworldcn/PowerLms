...
graph TD
    ...
    subgraph "基础设施组件"
        ...
        M[ActualFinancialTransaction<br/>实际收付记录]
    end
```
...
| **实际收付记录** | `ActualFinancialTransaction` | PowerLmsData | 分次收付款记录，支持通用父单据关联 |
...
// 分次收付 → 使用 ActualFinancialTransaction
```
...
- **财务管理**: 结算单(PlInvoices)、发票、费用方案、凭证生成、AR/AP财务编码对接
...
### 🎯 当前开发重点：主营业务结算单(PlInvoices)功能深度改造

#### 重大发现与技术评估
通过对现有PowerLms架构的深入分析，发现了显著优于预期的技术基础：

- ✅ **PlInvoices实体已有65%字段**：现有17个字段完全匹配需求
- ✅ **ActualFinancialTransaction实体完全满足**：支持分次收付需求，无需新建表
- ✅ **FinancialController API功能完备**：完整的CRUD和确认流程
- ✅ **现有前端界面可扩展**：无需重新开发

#### 字段扩展计划
根据会议纪要需求，PlInvoices实体需新增16个字段：

| 类别 | 字段数量 | 主要字段 |
|------|----------|----------|
| **财务字段** | 4个 | 财务支付确认、财务凭证号、支付方法、财务信息 |
| **汇率金额** | 8个 | 收付汇率、各类本位币金额、手续费等 |
| **业务字段** | 4个 | 回款单位、导出标识、预收付相关金额 |
...
#### 1. OA费用申请单Bug修复
- **问题**: OA备用申请单编辑保存失败
- **根因**: `OaExpenseController.ModifyOaExpenseRequisition`方法缺少`SaveChanges()`调用
- **解决**: 添加SaveChanges调用和日志记录
- **状态**: ✅ 已完成，功能恢复正常
...
- **实现范围**:
  - OA日常费用申请单：主单+明细分别锁定
  - 主营业务费用申请单：基于工作流状态实现锁定
- **核心技术**: 状态驱动锁定、分层权限控制、字段级保护
- **状态**: ✅ 已完成，逻辑完整

### 📋 正在进行的任务

#### 1. 主营业务结算单功能改造 [核心功能]
- **实体扩展**: PlInvoices新增16个字段（**预估2.5天**）
- **计算工具接口**: 核销金额等复杂计算的统一接口（**预估2天**）
- **通用收付记录**: 复用现有ActualFinancialTransaction（**已完成**）

#### 2. 财务导出接口开发 [紧急需求]
- **目标**: 完成两个财务单据导出接口
- **技术要点**: 使用现有任务调度机制，支持条件筛选
- **状态**: 📋 计划中（**预估2天**）
...
|----------|----------|----------|
| Bug修复与数据锁定 | 1天 | ✅ **已完成** |
| PlInvoices实体扩展 | 2.5天 | 📋 进行中 |
| 计算工具接口 | 2天 | 📋 计划中 |
| 财务导出接口 | 2天 | 📋 计划中 |

**总计工期**: 7.5天 | **已完成**: 1天 | **剩余**: 6.5天

### 技术风险控制
- **低风险**: 基于现有PlInvoices成熟架构扩展
- **兼容性**: 新字段均为可空类型，保持向后兼容
- **精度标准**: 金额2位小数，汇率4位小数
...
□ 分次收付需求 → 优先使用 ActualFinancialTransaction
□ 避免重复造轮子，确保系统一致性
```
...
*2025年技术重点：主营业务结算单深度改造，财务系统功能增强，基础设施持续优化*

**架构亮点**: API→Server→Data→OwDbBase→OwBaseCore 清晰分层，基础设施组件完备复用  
**当前状态**: Bug修复完成，主营业务结算单功能改造进行中，预计6.5天完成剩余开发任务
