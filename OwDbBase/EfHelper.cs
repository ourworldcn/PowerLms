...
namespace OW.Data
{
    ...
    public static class EfHelper
    {
        ...
        /// <summary>
        /// 从 Excel 表追加数据到指定类型的数据集。
        /// 通过反射自动获取实体主键进行重复检查，支持复合主键和代理类。
        /// </summary>
        /// <typeparam name="T">实体类型，必须有主键定义</typeparam>
        /// <param name="sheet">Excel 工作表，第一行应为属性名</param>
        /// <param name="db">数据库上下文</param>
        /// <param name="checkDuplicates">是否检查重复，默认true</param>
        /// <returns>成功添加的记录数</returns>
        /// <exception cref="InvalidOperationException">当实体没有主键时抛出</exception>
        public static int AddSheetToDbSet<T>(ISheet sheet, DbContext db, bool checkDuplicates = true) where T : class, new()
        {
            if (sheet == null) throw new ArgumentNullException(nameof(sheet));
            if (db == null) throw new ArgumentNullException(nameof(db));

            var dbSet = db.Set<T>();
            var excelData = NPOI.NpoiUnit.GetSheet<T>(sheet);
            if (!excelData.Any()) return 0;
            ...
            if (!checkDuplicates)
            {
                ...
                {
                    if (item is GuidKeyObjectBase guidEntity)
                        guidEntity.GenerateIdIfEmpty();
                    dbSet.Add(item);
                    addedCount++;
                }
                return addedCount;
            ...
            // 获取主键属性反射信息
            var keyPropertyInfos = keyProperties
            ...
            {
                var keyValues = keyPropertyInfos.Select(pi => pi.GetValue(entity)?.ToString() ?? "NULL");
            ...
            foreach (var item in dataList)
            {
                ...
                // 获取当前记录主键
                var itemKeyValues = keyPropertyInfos.Select(pi => pi.GetValue(item)?.ToString() ?? "NULL");
                var itemKeyString = string.Join("|", itemKeyValues);

                // 检查是否重复
                if (!existingKeys.Contains(itemKeyString))
                {
                    dbSet.Add(item);
                    existingKeys.Add(itemKeyString);
                    addedCount++;
                }
            }
        ...
        /// <returns>成功添加的记录数</returns>
        /// <exception cref="InvalidOperationException">当实体没有主键时抛出</exception>
        public static AddSheetToDbSetResult<T> AddSheetToDbSetWithAutoKey<T>(ISheet sheet, DbContext db, bool checkDuplicates = true) where T : class, new()
        {
            if (sheet == null) throw new ArgumentNullException(nameof(sheet));
            if (db == null) throw new ArgumentNullException(nameof(db));

            var result = new AddSheetToDbSetResult<T>();
            var dbSet = db.Set<T>();
            var excelData = NPOI.NpoiUnit.GetSheet<T>(sheet);
            ...
            {
                result.AddedCount = 0;
                result.SourceKeys = Array.Empty<object[]>();
                result.ExistingKeys = Array.Empty<object[]>();
                result.DatabaseOnlyKeys = Array.Empty<object[]>();
                return result;
            }
            ...
            // 获取源数据的主键集合
            var sourceKeys = new List<object[]>();
            var sourceKeyStrings = new HashSet<string>();
            ...
            foreach (var item in dataList)
            {
                ...
                var keyValues = keyPropertyInfos.Select(pi => pi.GetValue(item)).ToArray();
                sourceKeys.Add(keyValues);
                
                var keyString = string.Join("|", keyValues.Select(v => v?.ToString() ?? "NULL"));
                sourceKeyStrings.Add(keyString);
            }
            result.SourceKeys = sourceKeys.ToArray();
            ...
            var existingKeyStrings = new HashSet<string>();
            
            foreach (var entity in existingEntities)
            {
                var keyValues = keyPropertyInfos.Select(pi => pi.GetValue(entity)).ToArray();
                existingKeys.Add(keyValues);
                
                var keyString = string.Join("|", keyValues.Select(v => v?.ToString() ?? "NULL"));
                existingKeyStrings.Add(keyString);
            }
            result.ExistingKeys = existingKeys.ToArray();

            // 找出数据库中存在但源中不存在的ID集合
            var databaseOnlyKeys = new List<object[]>();
            foreach (var entity in existingEntities)
            {
                var keyValues = keyPropertyInfos.Select(pi => pi.GetValue(entity)).ToArray();
                var keyString = string.Join("|", keyValues.Select(v => v?.ToString() ?? "NULL"));
                
                if (!sourceKeyStrings.Contains(keyString))
                {
                    databaseOnlyKeys.Add(keyValues);
                }
            }
            result.DatabaseOnlyKeys = databaseOnlyKeys.ToArray();

            // 添加不重复的记录
            foreach (var item in dataList)
            {
                var itemKeyValues = keyPropertyInfos.Select(pi => pi.GetValue(item)).ToArray();
                var itemKeyString = string.Join("|", itemKeyValues.Select(v => v?.ToString() ?? "NULL"));

                // 检查是否重复
                if (!existingKeyStrings.Contains(itemKeyString))
                {
                    dbSet.Add(item);
                    existingKeyStrings.Add(itemKeyString); // 避免同批次重复
                    result.AddedCount++;
                }
            }
        ...
        /// <summary>
        /// 增强版本：从 Excel 表追加数据到指定类型的数据集，并返回详细的ID分析结果。
        /// 可以获取源ID集合、数据库中存在但源中不存在的ID集合等信息。
        /// </summary>
        /// <typeparam name="T">实体类型，必须有主键定义</typeparam>
        /// <param name="sheet">Excel 工作表，第一行应为属性名</param>
        /// <param name="db">数据库上下文</param>
        /// <param name="checkDuplicates">是否检查重复，默认true</param>
        /// <returns>包含详细操作信息的结果对象</returns>
        public static AddSheetToDbSetResult<T> AddSheetToDbSetWithKeyAnalysis<T>(ISheet sheet, DbContext db, bool checkDuplicates = true) where T : class, new()
        {
            if (sheet == null) throw new ArgumentNullException(nameof(sheet));
            if (db == null) throw new ArgumentNullException(nameof(db));
            ...
            else
            {
                ...
                foreach (var item in dataList)
                {
                    ...
                    {
                        dbSet.Add(item);
                        existingKeyStrings.Add(itemKeyString);
                        result.AddedCount++;
                    }
                }
            }
        ...
        /// <summary>
        /// AddSheetToDbSet 方法的返回结果类，包含详细的ID分析信息
        /// </summary>
        /// <typeparam name="T">实体类型</typeparam>
        public class AddSheetToDbSetResult<T> where T : class
        {
            /// <summary>
            /// 成功添加的记录数
            /// </summary>
            ...
            /// 源数据的主键集合
            /// </summary>
            public object[][] SourceKeys { get; set; } = Array.Empty<object[]>();

            /// <summary>
            /// 数据库中现有记录的主键集合
            /// </summary>
            public object[][] ExistingKeys { get; set; } = Array.Empty<object[]>();

            /// <summary>
            /// 数据库中存在但源中不存在的主键集合
            /// </summary>
            public object[][] DatabaseOnlyKeys { get; set; } = Array.Empty<object[]>();

            /// <summary>
            /// 获取数据库中存在但源中不存在的主键数量
            /// </summary>
            public int DatabaseOnlyCount => DatabaseOnlyKeys.Length;

            /// <summary>
            /// 获取源数据中的主键数量
            /// </summary>
            public int SourceCount => SourceKeys.Length;

            /// <summary>
            /// 获取数据库中现有记录的数量
            /// </summary>
            public int ExistingCount => ExistingKeys.Length;
        }
    }
}
