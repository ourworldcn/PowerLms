...
| `MoveState` | 移动组件 | 64 字节 | `MoveState.cs` |
| `ColliderState` | 碰撞组件 | 20 字节 | `ColliderState.cs` |
...
// ⭐ 定期压缩（批量删除已标记的实体）
if (frameCount % 120 == 0)  // 每 120 帧（2 秒）压缩一次
{
    int deleted = actorManager.Compact();  // ⭐ 使用高性能 IRefPredicate 批量压缩
    Console.WriteLine($"压缩删除了 {deleted} 个实体");
}

// ⚠️ 立即删除（不推荐，除非必要）
actorManager.DestroyActor(playerId);  // 会导致内存移动
```
...
### 问题 1：RemoveActor 未删除碰撞组件

**问题描述**：删除 Actor 时，只删除了 `Actors` 和 `Moves`，未删除 `Colliders` 和 `ColliderShapes`。
...
{
    // 1. 清理 ColliderShapes
    if (index < Colliders.Count)
    {
        ref var collider = ref Colliders.AsSpan()[index];
        if (collider.ShapeCount > 0)
        {
            ColliderShapes.RemoveRange(collider.ShapeStart, collider.ShapeCount);
            
            // 修正后续 Collider 的 ShapeStart
            var colliderSpan = Colliders.AsSpan();
...
### 问题 2：CollisionService 未实现边界碰撞

**问题描述**：`BoundaryMask` 标志已定义，但 `CollisionService` 未处理场景边界碰撞。

**修复方案**：在 `DetectCollisions()` 中添加边界检测阶段：
...
public void DetectCollisions()
{
    ...
    for (int i = 0; i < colliders.Length; i++)
    {
        ...
        if (col.BoundaryMask == BoundaryMask.None)
            continue;
        
        ref var move = ref moves[i];
        Vector2 pos = move.Position;
        
        // 获取最大半径
        float maxRadius = 0f;
        for (int j = 0; j < col.ShapeCount; j++)
        {
            float r = shapes[col.ShapeStart + j].Radius;
            if (r > maxRadius) maxRadius = r;
        }
        
        // 边界检测
        if ((col.BoundaryMask & BoundaryMask.Left) != 0 && 
            pos.X - maxRadius < scene.MinX)
        {
            move.Position.X = scene.MinX + maxRadius;
            move.Velocity.X = 0;
        }
        // ... 其他边界检测 ...
    }
...
| 组件 | 大小 | 对齐 | 说明 |
|------|------|------|------|
| `Actor` | class | N/A | 引用类型，支持继承 |
| `MoveState` | 64 字节 | 8 字节 | Cache Line 友好 |
| `ColliderState` | 16 字节 | 2 字节 | 紧凑布局 |
| `ColliderShape` | 12 字节 | 4 字节 | Vector2 + float |
