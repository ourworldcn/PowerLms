...
- 查找现有代码保持风格统一
- 复用已有基础设施组件
- **数据库迁移由开发人员手工执行，AI绝对不生成迁移文件，不执行Add-Migration或Update-Database命令**
...
### 3.2 实体命名约束（强制执行）
- **禁止Type后缀**：数据库实体类名不要以`Type`结尾（如 ~~`CustomerType`~~，应为 `Customer` 或 `CustomerCategory`）
- **禁止Class后缀**：数据库实体类名不要以`Class`结尾（如 ~~`ProductClass`~~，应为 `Product` 或 `ProductCategory`）
- **语义化命名**：实体名应直接表达业务概念，避免技术性后缀
- **分类表命名**：表达分类/类型含义时使用 `Category`、`Kind`、`Group` 等后缀
...
- 每次只执行一条命令，禁止在同一次调用中拼接多条命令；执行出错后重新执行同一条命令，不得跳过
